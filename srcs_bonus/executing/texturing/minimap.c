...
/*   Updated: 2022/11/21 19:27:36 by nsartral         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_cubes_one(t_game *zz, int colors[5], int side)
{
	...
	while (++i < (int)zz->ylines)
	{
		...
		{
			if (zz->map[i] && (int)ft_strlen(zz->map[i]) > j && \
				zz->map[i][j] == '6')
			{
				k = -1;
				while (++k < side)
				{
					l = -1;
					while (++l < side)
						zz->data.addr[(l + (side * i)) * zz->data.line_length
							/ 4 + k + (j * side)] = colors[0];
				}
			}
		}
		...
		while (++j < (int)zz->xlines)
		{
			...
			{	
				...
				while (++k < side)
				{
					...
					while (++l < side)
						zz->data.addr[(l + (side * i)) * zz->data.line_length
							/ 4 + k + (j * side)] = colors[zz->map[i][j] - 48];
				}
			}
		}
	}
...
void	print_minimap(t_game *zz)
{
	...
	x_size = (zz->rx / 4) / zz->xlines - 1;
	y_size = (zz->ry / 4) / zz->ylines - 1;
